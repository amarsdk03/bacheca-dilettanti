			...
			{list.items.map((profile) => (
				<Card key={profile.id}>
					<CardHeader>
						...
						<CardTitle>
							<Link href={`/dettagli-profilo?${new URLSearchParams({id: profile.id, type: profile.type})}`}>{profile.title}</Link>
						</CardTitle>
						<CardDescription>{PROFILE_OPTIONS.find(({value}) => value === profile.type)?.label}</CardDescription>
					</CardHeader>
					{following && <CardFooter>
	...
	return (
		<section aria-labelledby="relationships-heading" className="flex flex-col gap-6">
			<div>
				...
				<p className="mt-2 text-sm text-muted-foreground">Le persone che ti seguono e i profili che vuoi ritrovare.</p>
			</div>
			<Tabs defaultValue="followers" className="gap-6">
				<TabsList aria-label="Relazioni del profilo">
					<TabsTrigger value="followers">Follower {followers.status === "success" && <Badge variant="secondary">{followers.items.length}</Badge>}</TabsTrigger>
					<TabsTrigger value="following">Profili seguiti {following.status === "success" && <Badge variant="secondary">{following.items.length}</Badge>}</TabsTrigger>
				</TabsList>
	...
	return (
		<section aria-labelledby="saved-announcements-heading" className="flex flex-col gap-6">
			<div>
				...
				<p className="mt-2 text-sm text-muted-foreground">I tuoi annunci salvati, dal più recente. Compaiono solo quelli ancora pubblici.</p>
			</div>
			{list.status === "error" ? <LoadError /> : list.items.length === 0 ? (
				...
				<div className="grid gap-6 xl:grid-cols-2">
					{list.items.map(({announcement, savedAt}) => (
						<div key={announcement.id} className="flex min-w-0 flex-col gap-3">
